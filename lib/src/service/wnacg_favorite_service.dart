...
class WnacgFavoriteService
  ...
  final Map<int, _WnacgFavoriteEntry> _favoritesByGid =
      <int, _WnacgFavoriteEntry>{};
  ...
  void applyBeanConfig(String configString) {
    ...
    for (dynamic item in rawList) {
      ...
      _WnacgFavoriteEntry? entry =
          _WnacgFavoriteEntry.tryFromJson(item.cast<String, dynamic>());
      if (entry == null || !entry.gallery.galleryUrl.isWN) {
        continue;
      }
  ...
  String toConfigString() {
    List<_WnacgFavoriteEntry> sortedEntries = _favoritesByGid.values.toList()
  ...
  Future<void> addFavorite(
    ...
    if (!gallery.galleryUrl.isWN) {
      return;
    }

    _WnacgFavoriteEntry? existing = _favoritesByGid[gallery.gid];
    Gallery snapshot = _normalizeWnGallerySnapshot(gallery);

    _favoritesByGid[snapshot.gid] = _WnacgFavoriteEntry(
      gallery: snapshot,
      favoritedTime:
  ...
  List<Gallery> getDisplayFavorites({
    ...
    List<_WnacgFavoriteEntry> sortedEntries = _favoritesByGid.values
  ...
  Gallery _normalizeWnGallerySnapshot(Gallery source) {
  ...
  String _resolveDisplayTime(
      _WnacgFavoriteEntry entry, FavoriteSortOrder? sortOrder) {
  ...
  bool _matchesSearchConfig(
      _WnacgFavoriteEntry entry, SearchConfig? searchConfig) {
...
class _WnacgFavoriteEntry {
  ...
  static _WnacgFavoriteEntry? tryFromJson(Map<String, dynamic> map) {
    ...
    int favoriteCategoryIndex =
        int.tryParse(map['favoriteCategoryIndex']?.toString() ?? '') ??
            WnacgFavoriteService.defaultFavoriteCategoryIndex;
    if (favoriteCategoryIndex <
            WnacgFavoriteService.minFavoriteCategoryIndex ||
        favoriteCategoryIndex >
            WnacgFavoriteService.maxFavoriteCategoryIndex) {
      favoriteCategoryIndex =
          WnacgFavoriteService.defaultFavoriteCategoryIndex;
    }

    return _WnacgFavoriteEntry(
      gallery: gallery,
