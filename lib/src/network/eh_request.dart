...
''';
  ...
  List<String> _nhFallbackHosts(String preferredHost, {bool includeTo = true}) {
    List<String> hosts = [];
  ...
  String _nhCdnHost(String sourceHost, {required String prefix}) {
    String normalized = _nhNormalizeHost(sourceHost);
    if (normalized == 'nhentai.to') {
      return 'zrocdn.xyz';
    }
  ...
  Future<T> _requestNhGalleryPage<T>({
    ...
    List<String> fallbackHosts = _nhFallbackHosts(
      preferredHost,
      includeTo: !isPopularRequest && nhToQuery != null,
    );
  ...
  Map<String, dynamic> _extractNhToGalleryJson(String html) {
    ...
    String rawJson = _sanitizeJsJson(match.group(1)!);
  ...
  /// Remove trailing commas that are valid in JavaScript but invalid in JSON.
  /// e.g. {"key": "val",} → {"key": "val"} and [1, 2,] → [1, 2]
  String _sanitizeJsJson(String js) {
    return js.replaceAllMapped(
      RegExp(r',(\s*[}\]])'),
      (match) => match.group(1)!,
    );
  }
  ...
  String? _buildNhToQuery(SearchConfig? searchConfig,
    ...
    if (RegExp(r'^\d+$').hasMatch(defaultQuery)) {
      return defaultQuery;
    }

    List<String> parts = [];
    String? keyword = _normalizeNhToKeyword(searchConfig?.keyword);
    if (keyword != null && keyword.isNotEmpty) {
      parts.add(keyword);
    }
    ...
    if (searchConfig?.tags?.isNotEmpty ?? false) {
      for (TagData tag in searchConfig!.tags!) {
        String key = _stripTrailingExactMarker(tag.key.trim());
        if (key.isEmpty) {
          continue;
        }
        parts.add(_quoteNhTextIfNeeded(key));
      }
    }

    String? language = searchConfig?.language?.trim();
    if (language != null && language.isNotEmpty) {
      parts.add(_quoteNhTextIfNeeded(language));
    }

    String query = parts.join(' ').trim();
    if (query.isNotEmpty) {
      return query;
    }

    String fallback = _normalizeNhToKeyword(defaultQuery) ?? '';
  ...
  String? _normalizeNhToKeyword(String? rawKeyword) {
    if (rawKeyword == null) {
      return null;
    }
    ...
    for (RegExpMatch match in matches) {
      ...
      if (key != null) {
        ...
        tokens.add(_quoteNhTextIfNeeded(normalizedKey));
        continue;
      }
    ...
    if (tokens.isEmpty) {
      return RegExp(r'[:"]').hasMatch(keyword) ? '' : keyword;
    }
